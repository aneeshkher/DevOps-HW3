...
app.get('/meow', function(req, res) {
	...
	client.lpop("pictures", function(err, value) {
		client.lpop("pictureNames", function(err1, value1) {
			if (value == null) {
				res.write("<h1>No more images. Port 3001</h1>", function(err) {
					res.end();
				});
			} else {
				res.write("<h1>Image of " + value1 + " on port 3001</h1><br>" + 
...
app.get('/get', function(req, res) {
	client.get("key", function(err, value) {
		client.ttl("key", function(err1, value1) {	
			if (value == null) {
				value = "Already expired. Port: 3001";
			} else {
				value = "<b>" + value + "</b>" + ". Time remaining: <b>" + value1 +
					" seconds</b> at port 3001";
			}
			res.send(value);
		});
	});
...
app.get('/aneesh', function(req, res) {
	var fullUrl = req.protocol + '://' + req.get('host') + req.originalUrl;
	var myText = "<h3>You have reached the URL: " + req.url + " on port 3001</h3>";
	res.send(myText);
});
...
app.get('/state', function(req, res) {
	var myText = "<h3>You have reached the URL: " + req.url + "</h3>" +
		"<h4>State: State of North Carolina on port 3001</h4>";
		res.send(myText);
});
